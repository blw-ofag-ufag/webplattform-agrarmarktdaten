  ...
  create(context) {
    return {
      'CallExpression[callee.name="makeStyles"]': function (node) {
        ...
        if (!classNames || !classNames.properties || !classNames.properties.length) {
          return;
        }
        ...
        const checkProperty = (property) => {
          if (isShorthandProperty(property)) {
            context.report({
              node: property,
              message: "Cannot use shorthand properties in makeStyles, please use full property",
            });
          }
          if (property.key && property.key.name === "typography") {
            context.report({
              node: property,
              message: 'Cannot use "typography" property in makeStyles, please use sx for this',
            });
          }
          ...
          if (property.value.type === "Literal" && isThemeColor(property.value.value)) {
            context.report({
              node: property,
              message:
        ...
        while (items.length > 0) {
          ...
          for (let property of item.value.properties) {
            if (property.value.type === "ObjectExpression") {
              items.push(property);
              if (property.key && property.key.value && property.key.value.startsWith(":")) {
                context.report({
                  node: property,
