...
- **Content modeling** Instead of composing pages with generic content blocks, content is structured in domain-specific models (e.g. market areas, reports) that can be linked together and integrated flexibly by a frontend (or even multiple frontends with different functionality).
